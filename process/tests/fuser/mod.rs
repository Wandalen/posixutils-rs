...
/// Retrieves the username associated with the given process ID on macOS.
///
/// # Arguments
///
/// * `pid` - The process ID for which to retrieve the user information.
...
/// Returns a `Result<String, io::Error>` where:
/// - `Ok(String)` contains the username if the process and user information are successfully retrieved.
/// - `Err(io::Error)` contains an error if the process information cannot be retrieved or other issues occur.
fn get_process_user(pid: u32) -> io::Result<String> {
    // Buffer to hold process information
    let mut proc_info = osx_libproc_bindings::proc_bsdinfo {
        pb_flags: 0,
        pb_memsize: 0,
        pb_regsize: 0,
        pb_nice: 0,
    ...
    unsafe {
        let mut size = std::mem::size_of::<libc::proc_bsdinfo>() as u32;
        let result = libc::proc_pidinfo(pid, libc::PROC_PIDTBINFO, 0, &mut proc_info as *mut _ as *mut libc::c_void, &mut size);
        if result <= 0 {
            return Err(io::Error::new(io::ErrorKind::NotFound, "Failed to retrieve process information"));
        }

        let uid = proc_info.pbi_uid;
        let pwd = getpwuid(uid as uid_t);
        if pwd.is_null() {
            return Err(io::Error::new(io::ErrorKind::NotFound, "User not found for UID"));
        }
