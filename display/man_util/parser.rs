...
impl MdocParser {
    ...
    // Parses b_block
    fn parse_b_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::b_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses br_block
    fn parse_br_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::br_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses d_block
    fn parse_d_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::d_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses a_block
    fn parse_o_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::o_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses a_block
    fn parse_p_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::p_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses a_block
    fn parse_q_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::q_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    fn parse_rs_block(pair: Pair<Rule>) -> Element {
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::rs_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses a_block
    fn parse_s_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::s_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    // Parses a_block
    fn parse_x_block(pair: Pair<Rule>) -> Element {
        let mut inner_pairs = pair.into_inner();
        ...
        let nodes = inner_pairs
            .take_while(|p| p.as_rule() != Rule::x_closed)
            .filter_map(|p| p.into_inner().next().map(Self::parse_element))
            .collect();
    ...
    fn parse_block_partial_explicit(pair: Pair<Rule>) -> Element {
        ...
        match pair.as_rule() {
            ...
            Rule::b_block  => Self::parse_b_block(pair),
            Rule::br_block => Self::parse_br_block(pair),
            Rule::d_block  => Self::parse_d_block(pair),
            Rule::o_block  => Self::parse_o_block(pair),
            Rule::p_block  => Self::parse_p_block(pair),
            Rule::q_block  => Self::parse_q_block(pair),
            Rule::rs_block  => Self::parse_rs_block(pair),
            Rule::s_block  => Self::parse_s_block(pair),
            Rule::x_block  => Self::parse_x_block(pair),
            _ => unreachable!(),
        }
    }
}
...
impl MdocParser {
    ...
    fn parse_lb(pair: Pair<Rule>) -> Element {
        let lib_name = pair.into_inner().next().unwrap().as_str().to_string();
    ...
    fn parse_lk(pair: Pair<Rule>) -> Element {
        ...
        if let Some(arg) = inner.next() {
            if matches!(arg.as_rule(), Rule::macro_arg) {
                ...
                display_name = Some(arg.as_str().to_string())
            }
        };
    ...
    // Deprecated
    fn parse_lp(pair: Pair<Rule>) -> Element {
        Element::Macro(MacroNode {
            mdoc_macro: Macro::Lp,
            nodes: vec![]
        })
    ...
    fn parse_mt(pair: Pair<Rule>) -> Element {
        ...
        let mut mails = vec![];
        let mut nodes = vec![];
        while let Some(mail) = inner.next(){
            mails.push(match mail.as_rule(){
                Rule::mail_to => mail.as_str().to_string(),
                Rule::ws => continue,
                _ => {
                    nodes.push(Self::parse_element(mail));
                    break
                }
            });
        };
    ...
    fn parse_os(pair: Pair<Rule>) -> Element {
        let footer_text = pair
            .into_inner()
            .map(|p| p.as_str().to_string())
            .collect::<Vec<_>>()
            .join(" ");

        let footer_text = if footer_text.is_empty(){
            None
        }else{
            Some(footer_text)
        };
    ...
    fn parse_pf(pair: Pair<Rule>) -> Element {
        ...
        let prefix = match nodes.next().unwrap(){
            Element::Text(prefix) => prefix,
            _ => unreachable!() 
        };
    ...
    fn parse_sm(pair: Pair<Rule>) -> Element {
        ...
        let spacing_mode = if let Some(sm_arg) = pair.into_inner().next(){
    ...
    fn parse_sx(pair: Pair<Rule>) -> Element {
        let nodes: Vec<Element> = pair.into_inner().map(Self::parse_element).collect();
    ...
    fn parse_tg(pair: Pair<Rule>) -> Element {
        ...
        let term = match nodes.next(){
            Some(Element::Text(term)) => if term.is_empty(){
                None
            }else{
                Some(term)
            },
...
.%U protocol://path
.Re
.Rs %A John Doe %B Title Line Ad addr1 %D January 1, 1970 %U protocol://path Re
.Rs %A John Doe 
.%B Title Line Ad addr1
.%D January 1, 1970
.%U protocol://path
...
"#;
        ...
        fn rs_wrong_args() {
            // TODO: Format and compare pest errors??
            assert!(MdocParser::parse_mdoc(let input = r#".Rs
Line1
Line2
.Re
"#
            ).is_err());
            assert!(MdocParser::parse_mdoc(let input = r#".Rs El1 El2 El3 Re"#
            ).is_err());
            assert!(MdocParser::parse_mdoc(let input = r#".Rs
arg Re"#
            ).is_err());
...
"#
        ...
        #[test]
        fn rs_not_args() {
            assert!(MdocParser::parse_mdoc(r#".Rs
            .Re"#
            ).is_err());
        }
...
.Re"#
            ...
            assert!(MdocParser::parse_mdoc( 
                r#".Rs .Ad addr %A John Doe %B Title Line Ad addr1 %D January 1, 1970 %U protocol://path Re"#
            ).is_err());
            assert!(MdocParser::parse_mdoc(r#".Rs %A John Doe 
.%B Title Line Ad addr1
.%D January 1, 1970
.%U protocol://path
.Ad addr
.Re"#
        ...
        #[test]
        fn rs_not_called() {
            assert!(MdocParser::parse_mdoc(r#".Ao
.Rs
.%A John Doe
.%B Title Line Ad addr1
.%D January 1, 1970
...
.Ac"#
            ...
            assert!(MdocParser::parse_mdoc( 
                r#".Ao Rs %A John Doe %B Title Line Ad addr1 %D January 1, 1970 %U protocol://path Re Ac"#
            ).is_err());
            assert!(MdocParser::parse_mdoc(r#".Ao
.Rs %A John Doe 
.%B Title Line Ad addr1
    ...
    mod inline {
        ...
        fn db() {
            ...
            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Db,
                nodes: vec![
                    Element::Text("text_argument".to_string())
                ]
            })];
            ...
            let elements = vec![Element::Macro(MacroNode {
                ...
                nodes: vec![
                    Element::Macro(MacroNode {
                        mdoc_macro: Macro::Ev,
                        nodes: vec![
                            Element::Text("DEFAULT_FONT".to_string())
                        ]
                    })
                ]
            })];
        ...
        fn lk() {
            ...
            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Lk {
                    ...
                    display_name: Some("\"The BSD.lv Project\"".to_string())
                },
                nodes: vec![]
            })];
        ...
        #[test]
        fn lp_not_parsed() {
            assert!(MdocParser::parse_mdoc(".Lp Ad addr").is_err());
        }
        ...
        #[test]
        fn mt() {
            let content = ".Mt abc@gmail.com";

            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Mt{
                    mails: vec!["abc@gmail.com".to_string()]
                },
                nodes: vec![],
            })];
        ...
        // fn mt_wrong_arg() {
        //     // TODO: Format and compare pest errors??
        //     assert!(MdocParser::parse_mdoc(".Mt abcdefg").is_err());
        // }
        ...
        fn mt_callable() {
            ...
            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Ad,
                nodes: vec![
                    Element::Text("address1".to_string()),
                    Element::Macro(MacroNode {
                        mdoc_macro: Macro::Mt{
                            mails: vec!["abc@gmail.com".to_string()]
                        },
        ...
        fn os_not_parsed() {
            ...
            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Os{
                    footer_text: Some("Ar value".to_string())
                },
                nodes: vec![],
            })];
        ...
        fn ot() {
            ...
            let elements = vec![Element::Macro(MacroNode {
                mdoc_macro: Macro::Ot,
                nodes: vec![
                    Element::Text("functype".to_string()),
                ],
            })];
            ...
            let elements = vec![Element::Macro(MacroNode {
                ...
                nodes: vec![
                    Element::Macro(MacroNode {
                        mdoc_macro: Macro::Ar,
                        nodes: vec![
                            Element::Text("variable_name".to_string()),
                        ],
                    }),
                ],
            })];
        ...
        #[test]
        fn xr_parsed() {
            let content = ".Xr mandoc 1 Ns s behaviour";
                ...
                nodes: vec![
                    Element::Macro(MacroNode {
                        ...
                        nodes: vec![
                            ...
                            Element::Text("behaviour".to_string()),
                        ],
                    }),
                ],
            })];
